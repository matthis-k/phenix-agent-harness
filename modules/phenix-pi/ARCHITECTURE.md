...
A user follow-up received while descendants are active is recorded as `run.input.amended` and consumed by `AttentionProcessManager`. The process manager is a root-scoped supervisory control plane; it is not a workflow node and does not alter immutable workflow inputs or graphs.

The manager derives a bounded candidate set from active agent projections and invokes the internal no-tools `agent.attention-router`. That definition is authorized for runtime use but excluded from model-facing catalogs and ordinary delegation capabilities. The router may select only offered run IDs and may return no targets when the root supervisor should handle the message.

Urgent attention is delivered immediately through the existing execution/session boundary. A streaming child is steered without waiting for its current cycle to finish. A non-streaming child receives a new message cycle. Next-turn context is delivered through the session notification path. Existing awaits, ownership edges, cancellation propagation, and typed completion remain unchanged.

Delivery is persisted through `attention.received`, `attention.routed`, `attention.delivery.deferred`, `attention.delivered`, and failure events. Messages targeting a child that is still starting are deferred, reconstructed from the ledger after restart, and delivered when the child session binds. Workflow runs are not direct attention targets; graph-changing follow-ups remain root-supervisor decisions.
...
The composition layer subscribes to canonical domain events and maps them to stable lowercase dot-separated diagnostic scopes. Runtime, integration, persistence, agent-session, model-resolution, workflow, tool, output, failure, recovery, and attention boundaries may also record explicit diagnostics when the domain event alone lacks enough context.
