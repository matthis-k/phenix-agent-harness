...
describe("Routing matrix cells (9 roles × 4 difficulties)", () => {
  it("every role/difficulty pair resolves to expected capability and thinking", () => {
    const expected: Record<
      RoutingRole,
      Record<Difficulty, { capability: Capability; thinking: string }>
    > = {
      coordinator: {
    ...
    for (const role of ALL_ROLES) {
      for (const difficulty of ALL_DIFFICULTIES) {
        ...
        assert.equal(
          route.capability,
          expect.capability,
          `${role}/${difficulty} capability mismatch`,
        );
    ...
    it(`${setId} every pool candidate parses as provider/model`, () => {
      ...
      for (const cap of [
        ...
        for (const candidate of pool) {
          ...
          assert.ok(
            slash > 0 && slash < candidate.length - 1,
            `Malformed candidate "${candidate}" in ${poolName}`,
          );
        }
      }
    });
