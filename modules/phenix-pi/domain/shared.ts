...
export type SuccessfulOutcome<O> = Extract<Outcome<O>, { readonly status: "success" }>;
export type FailedOutcome = Extract<Outcome<never>, { readonly status: "failure" }>;
export type CancelledOutcome = Extract<Outcome<never>, { readonly status: "cancelled" }>;

export function success<O>(value: O): SuccessfulOutcome<O> {
  return { status: "success", value };
}

export function failed<O = never>(failure: Failure): FailedOutcome {
  return { status: "failure", failure };
}

export function cancelled<O = never>(reason: string): CancelledOutcome {
