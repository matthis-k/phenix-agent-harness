...
test("accepts RTK advisory rewrites and scopes exact recovery per tool call", async () => {
  ...
  assert.match(preparation.command, /RTK_TEE=0/);
  ...
  await writeFile(path.join(directory, "phenix-raw.log"), "complete command output");
...
test("passes composed shell commands through unchanged", async () => {
  const backend = new ProcessRtkTokenReductionBackend({
    executable: "rtk",
    stateDirectory: await mkdtemp(path.join(os.tmpdir(), "phenix-rtk-")),
    execute: executor(0, "rtk git status && rtk cargo test"),
  });
