...
} from "./runtime/child-session-backend.ts";
import { createSessionSubagentManagerFactory } from "./runtime/subagent-manager-factory.ts";
...
} from "./subagents/managed-delegation-runtime.ts";
...
export default async function phenix(pi: ExtensionAPI): Promise<void> {
  const suiteConfiguration = loadPhenixSuiteConfiguration();
  registerOutputSchemas(defaultOutputSchemas);
  registerOutputSchemas(outputSchemasFromContracts(suiteConfiguration.composition.contracts));
  registerWorkflowDefinitions(suiteConfiguration.workflows);
  configureRoutingConfig(
    buildRoutingConfigFromDeclarations({
      routing: suiteConfiguration.composition.routing,
      defaultModelSet: suiteConfiguration.composition.activeModelSet.id,
    }),
  );

  const linkResult = link(suiteConfiguration.composition);
