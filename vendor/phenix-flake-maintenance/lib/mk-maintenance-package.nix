...
let
  inherit (builtins) attrNames concatLists isFunction isList map;

  fail = message: throw "phenix-flake-maintenance: ${message}";

  collectRuntimeInputs = path: node:
    let
      ...
      nested = concatLists (
        map
          (name: collectRuntimeInputs (path ++ [ name ]) children.${name})
          (attrNames children)
      );
    in
    if !isList resolved then
      fail "`${builtins.concatStringsSep " " ([ maintenance.name ] ++ path)}`: runtimeInputs must resolve to a list"
    else
      resolved ++ nested;

  runtimeInputs = concatLists (
    map
      (name: collectRuntimeInputs [ name ] maintenance.commands.${name})
