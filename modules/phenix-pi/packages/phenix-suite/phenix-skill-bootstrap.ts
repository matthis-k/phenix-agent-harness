...
const PHENIX_CODING_GUIDANCE = [
  ...
  "The runtime resolves workflow authority before inference and injects the target agents available from the current node into the system prompt. When required transitions exist, the runtime blocks every non-workflow execution tool until `phenix_workflow` succeeds with `action: spawn` and one required target agent. After each successful spawn, authority is re-evaluated and any next required transition is enforced. Use `action: inspect` only when no required transition is pending and a prior workflow action may have changed optional authority.",
