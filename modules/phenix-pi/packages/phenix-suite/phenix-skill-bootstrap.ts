...
const PHENIX_CODING_GUIDANCE = [
  ...
  "The runtime resolves workflow authority before inference and injects the target agents available from the current node into the system prompt. Use `phenix_workflow` with `action: inspect` only when a prior workflow action may have changed that authority. Use `phenix_workflow` with `action: spawn`, one currently advertised `agent`, and a bounded task to create a child. Never provide a node ID, transition ID, role, model, tool set, or direct child-session configuration.",
  "When the user explicitly requests subagents, delegation, or the Phenix workflow, do not proceed as a single agent: make `phenix_workflow` spawning the first substantive execution action. If it fails, report the exact runtime/provider failure rather than silently continuing solo.",
