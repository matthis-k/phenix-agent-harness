...
```phenix-tools
allow: read, grep, find, ls, phenix_run, phenix_handle, phenix_tasks, phenix_project, phenix_present, phenix_visualize
```
...
When assigned a Phenix project decision, claim it before work, resolve only that decision, record rationale/evidence/consequences, and use `phenix_project` `request_input` when a human decision is required. Do not infer the human's answer.
