...
test("QA analysis agents do not delegate command execution to scouts", () => {
  ...
  assert.match(
    architectDefinition.prompt.render(),
    /deterministic checks are handled by a separate tester branch/i,
  );
  assert.match(
    architectDefinition.prompt.render(),
    /Delegate to agent\.scout only for a focused repository evidence question/i,
  );
});
