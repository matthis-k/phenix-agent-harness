...
import type { WorkflowActorSource } from "@matthis-k/phenix-flow/workflow-runtime.ts";
import {
...
import { modelSetForModelId, PHENIX_PROVIDER } from "@matthis-k/phenix-routing/provider.ts";
import { childRunId } from "../runtime/child-session-types.ts";
...
export class WorkflowDelegator {
  ...
  private async delegateInternal(input: {
    ...
    const transition = dependencies.definition.transitions.find((item) => item.id === transitionId);
    if (!transition) {
      return {
        ok: false,
