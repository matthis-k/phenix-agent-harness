...
| Area | Required work |
|---|---|
| Asynchronous aggregate updates | emit `_phenix/node/event` and `_phenix/session_tree/updated` notifications without requiring explicit polling |
...
| Standard session persistence | implement standard `session/list`, `load`, `resume`, `fork`, and delete over Phenix tree roots |
