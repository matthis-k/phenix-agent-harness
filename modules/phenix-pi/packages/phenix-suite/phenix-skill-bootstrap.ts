...
import type { ModelIdentity } from "./composition/model-scope.ts";
...
const PHENIX_CODING_GUIDANCE = [
  ...
  "The runtime resolves workflow authority before inference and injects the target agents available from the current node into the system prompt. Required SKILL.md files may be read locally before delegation. Do not delegate skill loading, contract loading, workflow inspection, or other Phenix harness preflight. When required transitions exist, the runtime blocks repository execution until `phenix_workflow` is called with `action: spawn` and one required target agent. The model task may state a useful focus, but required root assignments are bound by the runtime to the complete user request and transition purpose. After every spawn result, authority is re-evaluated; a terminal workflow failure blocks further execution for that turn instead of advertising stale targets. Use `action: inspect` only when no required transition is pending and a prior workflow action may have changed optional authority.",
  "When the user explicitly requests subagents, delegation, or the Phenix workflow, do not proceed as a single agent. If workflow spawning fails, report the exact runtime/provider failure rather than silently continuing solo.",
  "`phenix_subagent` is available only when the current authority explicitly includes it in effectiveTools; it remains workflow-backed. Raw `subagent` is runtime-blocked.",
