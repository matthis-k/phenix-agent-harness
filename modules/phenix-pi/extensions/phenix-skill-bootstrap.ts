...
const PHENIX_CODING_GUIDANCE = [
  ...
  "The runtime resolves workflow authority before inference and injects the target agents available from the current node into the system prompt. Use `phenix_workflow` with `action: spawn`, one advertised `agent`, and a bounded task. Never provide a node ID, transition ID, role, model, tool set, or direct child-session configuration. Raw `subagent` and legacy `phenix_delegate` are runtime-blocked.",
